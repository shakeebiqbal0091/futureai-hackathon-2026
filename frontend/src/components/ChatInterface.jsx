...
const ChatInterface = ({ apiData, onAnalysisComplete }) => {
  ...
  const sendMessage = async () => {
    if (!inputValue.trim() || !apiData) return;
          ...
          setTimeout(() => {
            onAnalysisComplete({
              ...
              automation_opportunities: [
                { task_column: 'Data Entry', annual_time_savings: 2000 },
                { task_column: 'Report Generation', annual_time_savings: 1500 },
                { task_column: 'File Processing', annual_time_savings: 1000 }
              ]
            });
          }, 1000);
        }
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-6">
      <h2 className="text-xl font-semibold mb-4">Chat with WFIA Agent</h2>
      ...
      <div className="h-96 overflow-y-auto mb-4 bg-gray-50 rounded-lg p-4">
        {messages.length === 0 ? (
          <p className="text-gray-500 text-center py-8">
            Start by uploading data, then ask questions like:
            <br className="mb-2" />
            "What insights can you see in the data?"
            <br />
            "Show me FTE savings analysis"
            <br />
      ...
      {!apiData && (
        <div className="mt-4 text-sm text-gray-500">
          <p>Please upload data first to enable chat functionality.</p>
        </div>
      )}
    </div>
  );
